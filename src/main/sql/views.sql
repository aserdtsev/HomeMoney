...
drop view public.v_trns_by_base_crn;
create or replace view public.v_trns_by_base_crn AS
  select
    mo.balance_sheet_id as bs_id,
    mo.status,
    mo.trn_date,
    mo.period,
    mo.recurrence_id,
    ...
    round(mo.amount * coalesce(erf.ask, coalesce(ert.ask, 1)), 2) as amount
  from
    money_oper mo,
    accounts af
  ...
  where af.id = mo.from_acc_id and at.id = mo.to_acc_id;
